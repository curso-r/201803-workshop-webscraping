...
No navegador, inspecione o [http://example.webscraping.com/](http://example.webscraping.com/) e identifique uma tabela no corpo do site. Em seguida, utilize a função `html_table()` e compare o resultado com o observado no inspetor. Qual conteúdo a função devolveu: tag, texto ou atributos?
...
Até aqui o pacote httr já nos ajudou muito.

- Finalidade: mexer com requisições e respostas HTTP.
- Principais funções: GET, POST, content, cookies, write_disk, status_code.

Exercício: explore por cima a lista de todas as funções do `httr`. Tente achar uma função que o professor não saiba para ferrar com ele.
